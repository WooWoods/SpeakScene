...
SpeakScene 是面向中文母语者的 AI 情景英语陪练。核心不是让用户把指定中文句子翻译成英文，而是在一个真实场景中先获得常用表达，再由系统主动发起对话，帮助用户用语音、键入或手写整理的方式完成一段可持续的英语互动。
...
- 中列：AI 对话区。系统先发起对话，页面打印英文 starter，并通过浏览器 TTS 朗读。用户可通过语音、键入或手写板整理文本后回复。所有轮次都保存为对话记录。
...
- 手写输入 MVP：提供 canvas 手写板和文本整理框，用户将手写内容整理成英文后提交；暂不做 OCR。
...
2. 接入后端 STT/TTS：在需要更稳定语音体验时增加 provider abstraction。
...
- 系统开场白会显示并可朗读。
- 用户至少可以通过文本输入完成对话；支持的浏览器可使用语音识别；手写板可辅助整理文本。
- 完成对话后能看到多维度评分和建议表达。
- 收藏表达能按场景分类展示，并可删除。
